      ...
      >
        <template #default>
          <NTSearchFormFilterItem>
            <el-form-item label="变量名称:" prop="var_key">
              <el-input
                v-model.trim="searchFormFilter.var_key"
                clearable
                placeholder="请输入变量名称"
                @clear="handleSearch"
              />
          ...
          <NTSearchFormFilterItem>
            <el-form-item label="变量标识:" prop="var_name">
              <el-input
                v-model.trim="searchFormFilter.var_name"
                clearable
                placeholder="请输入变量标识"
                @clear="handleSearch"
              />
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataList = async () => {
      ...
      const apiRes = await $api.teplateVar
        .tplVarListApi(params)
        .catch((error) => {
          $message.error({
            message: error,
            duration: 3000,
    ...
    // 添加
    const handleAdd = () => {
      router.push({ path: '/publicVar/template/add' })
    }

    // 编辑
    const handleEdit = ({ id }) => {
      router.push({ path: '/publicVar/template/edit', query: { id } })
    }

    // 详情
    const handleDetail = ({ id }) => {
      router.push({ path: '/publicVar/template/detail', query: { id } })
    }
