  ...
  <div class="page-container page-container--bg">
    <el-card v-loading="loadding" shadow="never" class="border-0">
      ...
      >
        ...
        <el-form-item prop="item_var">
          <template #label>
            item子项
            <el-popover placement="top-start" width="auto" trigger="hover">
              <template #default>
                <p class="ml-[12px]">list列表对应的item子项html代码</p>
              </template>
              <template #reference>
                <i class="ri-question-line ml-[4px]"></i>
              </template>
          ...
          <VAceEditor
            :value="formMdl.item_var"
            lang="html"
            theme="chrome"
            class="sm:w-1/1 md:w-1/1 lg:w-8/9 xl:w-8/9 2xl:w-2/3"
            @init="vaceItemVarEditorInit"
          >
          </VAceEditor>
        </el-form-item>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const vaceItemVarEditorRef = shallowRef()
    const vaceItemVarEditorInit = (instance) => {
      vaceItemVarEditorRef.value = instance

      instance.getSession().on('change', () => {
        formMdl.value.item_var = instance.getValue()
      })
    }
    ...
    const handleSubmit = async () => {
      ...
      const data = {
        ...
        item_var: vaceItemVarEditorRef.value.getValue(),
      }
      ...
      if (isCreateMode.value) {
        loadding.value = true
        await addListTemplate(formData)
        loadding.value = false
      } else if (isEditMode.value) {
        formData.append('id', formMdl.value.id)
        loadding.value = true
        await editListTemplate(formData)
        loadding.value = false
      }
    }
    ...
    // 添加
    const addListTemplate = async (params) => {
      const apiRes = await $api.listTemplate.addTplApi(params)
    ...
    // 编辑
    const editListTemplate = async (params) => {
      const apiRes = await $api.listTemplate.editTplApi(params)
      const { status, message } = apiRes.data
      if (status === $apiCode.SUCCESS) {
        isNeedRefreshPage.value = true
    ...
    const getDetail = async (id) => {
      ...
      const apiRes = await $api.listTemplate.tplDetailApi(params)
      const { status, data, message } = apiRes.data
      if (status === $apiCode.SUCCESS) {
        const { content, item_var: itemVar, ...extra } = data
        formMdl.value = {
          ...extra,
          ...
          item_var: beautify.html(itemVar, {
            indent_size: 2,
            indent_char: ' ',
            max_preserve_newline: 0,
            preserve_newlines: false,
