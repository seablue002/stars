...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataList = async () => {
      ...
      const apiRes = await $api.detailTemplate
        .tplListApi(params)
        .catch((error) => {
          $message.error({
            message: error,
    ...
    // 添加
    const handleAdd = () => {
      router.push({ path: '/template/detailTpl/add' })
    }

    // 编辑
    const handleEdit = ({ id }) => {
      router.push({ path: '/template/detailTpl/edit', query: { id } })
    }

    // 详情
    const handleDetail = ({ id }) => {
      router.push({ path: '/template/detailTpl/detail', query: { id } })
    }
