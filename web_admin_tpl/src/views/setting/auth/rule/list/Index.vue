    ...
    <ListTable
      ...
      <template #opts>
        <el-button
          type="primary"
          icon="plus"
          @click.stop="$router.push({ path: '/setting/auth/rule/add' })"
        >
          添加规则
        </el-button>
      </template>
...
export default defineComponent({
  ...
  setup () {
    ...
    const getRuleList = async () => {
      const params: RuleListProps = {
        name: searchForm.name,
        page: tbData.pageNo,
        size: tbData.pageSize
      }
      tbIsLoading.value = true
      const { status, data, message } = await ruleListApi(params)
    ...
    const handleEdit = ({ data: { id } }: any) => {
      router.push({ path: '/setting/auth/rule/edit', query: { id } })
    }
    useFixKeepAliveListRefresh.executeRefreshJudgment(getRuleList)
