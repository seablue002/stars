<template>
  <div class="add-info PAGE-MAIN-CONTENT">
    <PageHeader :title="hdTitle"></PageHeader>
    <keep-alive>
      <AddInfoForm ref="infoFormRef" :mode="mode"></AddInfoForm>
    </keep-alive>
  </div>
</template>
<script lang="ts">
...
import AddInfoForm from './components/AddInfoForm.vue'
  ...
  setup () {
    ...
    onMounted(() => {
      if (id.value && ['edit', 'detail'].includes(mode.value)) {
        infoFormRef.value.formMdl.id = id.value
      }
    })
    onActivated(() => {
      if (id.value && ['edit', 'detail'].includes(mode.value)) {
