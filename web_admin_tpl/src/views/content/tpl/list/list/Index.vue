    ...
    <ListTable
      ...
      <template #opts>
        <el-button
          type="primary"
          icon="plus"
          @click.stop="$router.push({ path: '/tpl/list/add' })"
        >
          添加列表模板
        </el-button>
      </template>
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleEdit = ({ data: { id } }: any) => {
      router.push({ path: '/tpl/list/edit', query: { id } })
    }

    // 详情
    const handleDetail = ({ data: { id } }: any) => {
      router.push({ path: '/tpl/list/detail', query: { id } })
    }
    ...
    const delListTpl = async (id: number) => {
    ...
    useFixKeepAliveListRefresh.executeRefreshJudgment(getListTplList)
